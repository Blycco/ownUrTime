...
> GitHub Issue: #10 | Branch: feat/feature-mood

## Domain Layer
- [x] lib/features/mood/domain/entities/mood_check.dart
  - freezed: id, userId, sessionId?, moodLevel (1–5), checkedAt
- [x] lib/features/mood/domain/repositories/mood_repository.dart
  - saveMoodCheck(userId, level, sessionId?) → Future<MoodCheck>
  - getTodayMoodChecks(userId) → Future<List<MoodCheck>>
- [x] lib/features/mood/domain/usecases/check_mood_usecase.dart
...
## Data Layer (Codex)
- [x] lib/features/mood/data/models/mood_check_model.dart
- [x] lib/features/mood/data/datasources/mood_remote_datasource.dart
- [x] lib/features/mood/data/repositories/mood_repository_impl.dart

## Presentation (Codex)
- [x] lib/features/mood/presentation/widgets/mood_check_widget.dart
...
- [x] Timing logic (in session provider or app lifecycle):
  - Show before first session of the day
  - Show after session ends (not before subsequent sessions in same day)

## Tests (Codex)
- [x] test/features/mood/domain/check_mood_usecase_test.dart
  - Test: level 1 → suggests 10 min
  - Test: level 2 → suggests 10 min
  - Test: level 3 → suggests 15 min
  - Test: level 5 → suggests 25 min
- [x] test/features/mood/presentation/mood_check_widget_test.dart
  - Test: 5 emoji options visible
  - Test: tapping one calls usecase with correct level
  - Test: skip option available

## Done When
- [x] flutter analyze — zero warnings in features/mood/
- [x] All tests pass
- [x] Mood 1–2 pre-fills 10 min in session screen
- [x] Appears before first daily session; after subsequent session completions
