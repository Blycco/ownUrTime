...
## Guest Mode (Claude Code — core flow)
- [ ] All Phase 1 features work without login:
  - Tasks stored in local state (not Supabase)
  - Sessions tracked locally
  - No network required for core flow
- [ ] Local storage: use flutter_secure_storage for task list + session history JSON
- [ ] GoRouter: no routes require auth in Phase 1 — auth is always opt-in

## Auth Prompt Trigger (Claude Code)
- [ ] After 3rd session completion → show sign-in bottom sheet
...
## Data Sync on Sign-In (Claude Code)
- [ ] On Apple Sign In success:
...
- [ ] Configure Apple Sign In in Supabase dashboard (done via console, document steps)
- [ ] lib/features/auth/data/datasources/auth_remote_datasource.dart
  - signInWithApple() → supabase.auth.signInWithOAuth(OAuthProvider.apple)
  - signOut() → supabase.auth.signOut()
  - getUser() → supabase.auth.currentUser
- [ ] Deep link handling for Apple OAuth callback (ios/Runner/Info.plist redirect URL)
...
- [ ] test/features/auth/presentation/sign_in_prompt_sheet_test.dart
  - Test: Apple Sign In button visible
  - Test: "Maybe later" dismisses sheet

## Done When
- [ ] flutter analyze — zero warnings in features/auth/
- [ ] All tests pass
- [ ] App launches directly to task list (no login screen)
- [ ] All features work in guest mode
- [ ] Sign-in prompt appears after exactly 3rd session (non-blocking)
- [ ] Sign in syncs local data to Supabase
